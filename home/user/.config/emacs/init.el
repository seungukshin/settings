...
;; exec path
(defun s/add-to-path (path)
  "Add PATH to both Emacs' `exec-path' and the system `PATH' environment variable."
  (when (file-directory-p path)
    (add-to-list 'exec-path path)
    (setenv "PATH" (concat path ":" (getenv "PATH")))))
(s/add-to-path (concat home-dir "bin/"))
(cond ((eq system-type 'windows-nt)
       (s/add-to-path (concat (getenv "MSYS2") "/usr/bin"))
       (cond ((string-match-p "x86_64" system-configuration)
	      (s/add-to-path (concat (getenv "MSYS2") "/mingw64/bin")))
	     ((string-match-p "aarch64" system-configuration)
	      (s/add-to-path (concat (getenv "MSYS2") "/clangarm64/bin"))
	      (s/add-to-path (concat (getenv "MSYS2") "/clangarm64/lib")))))
      ((eq system-type 'darwin)
       (s/add-to-path "/opt/homebrew/bin/")
       (setenv "LIBRARY_PATH"
	       (format "%s:%s:%s"
      ...
      ((eq system-type 'gnu/linux)
       (s/add-to-path "/var/run/host/bin")
       ...
       (cond ((string-match-p "x86_64" system-configuration)
	      (s/add-to-path (concat home-dir "bin-amd64/")))
	     ((string-match-p "aarch64" system-configuration)
	      (s/add-to-path (concat home-dir "bin-arm6464/"))))))
...
(use-package tab-bar
  ...
  (cl-loop for i from 1 to 8 do
	   ...
	   (which-key-add-key-based-replacements
	     (format "C-x t %d" i) (format "Select Tab %d" i))
	   (which-key-add-key-based-replacements
	     (format "C-c t %d" i) (format "Select Tab %d" i)))
  :bind
  (("C-x t g" . tab-select)
   ("C-x t t" . tab-recent)))
...
(use-package corfu
  ...
  (corfu-preview-current nil)	; do not pre-insert text into the line
  :bind
  (:map corfu-map
...
;;; terminal
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; sync the host clipboard with emacs kill ring
  ...
  (defun s/gnus-get-new-news ()
    ...
    (let ((sentinel
      ...
      (set-process-sentinel
       ...
       sentinel)))
  :config
  (setq gnus-directory (concat home-dir "Mail"))
  ;; user
...
(use-package ai-code
  ...
  :custom
  (ai-code-backends-infra-window-width 150)
  :config
  (setq ai-code-backends-infra-terminal-backend 'ghostel
	ai-code-session-link-enabled nil)
