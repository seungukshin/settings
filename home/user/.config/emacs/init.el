...
(cond ((eq system-type 'windows-nt)
       ...
       (add-to-list 'exec-path (concat (getenv "MSYS2") "/mingw64/usr/bin"))
       (add-to-list 'exec-path (concat (getenv "MSYS2") "/clangarm64/usr/bin"))
       (setenv "PATH" (format "%s:%s:%s:%s"
			      (concat (getenv "MSYS2") "/usr/bin")
			      (concat (getenv "MSYS2") "/mingw64/usr/bin")
			      (concat (getenv "MSYS2") "/clangarm64/usr/bin")
			      (getenv "PATH"))))
...
(setq tab-bar-tab-name-format-function
	  ...
	  (let* ((current-p (eq (car tab) 'current-tab))
		 ...
		 (title (format " %d%s %s%s " i
				(if (buffer-modified-p buffer) "*" "")
				name
				(if (and current-p dired-rsync-modeline-status)
...
(global-set-key (kbd "C-c t O") 'tab-previous)
...
(global-set-key (kbd "C-c t b") 'tab-switch)
(global-set-key (kbd "C-c t c") 'tab-new)
(global-set-key (kbd "C-c t k") 'tab-close)
(global-set-key (kbd "C-c t ,") 'tab-rename)
