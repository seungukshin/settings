...
wezterm.on('activate-tab', function(window, pane)
  ...
  for _, v in pairs({'amd', 'intel', 'arm1', 'arm2', 'arm3', 'arm4', 'arm5', 'arm6'}) do
    if title == v then
      local cmd = pane:get_foreground_process_name()
      if cmd and not cmd:find('ssh') then
	pane:send_text('ssh ' .. title .. ' -t "tmux at || tmux -2"\n')
      end
--[[
      local prompt, command, output
      prompt = '     ~ %s*'
      command = 'ssh ' .. v .. ' -t tmux list-sessions'
      if not exec_command(pane, prompt, command, 3) then
	wezterm.log_info('timeout')
	break
      end
      output = pane:get_logical_lines_as_text(pane:get_dimensions().scrollback_rows)
      result = {}
	...
	table.insert(result, line)
      ...
      local last = 0
      for i = #result, 1, -1 do
	if result[i]:find(add_escape(command)) then
	  last = i
	  break
	end
      end
      local exist = false
      for i = last, #result do
	if result[i]:find('%d+: %d+ window') then
	  wezterm.log_info(result[i])
	  exist = true
	  break
	end
      end
      if exist then
	exec_command(pane, prompt, 'ssh ' .. v .. ' -t tmux at', 3)
      else
	exec_command(pane, prompt, 'ssh ' .. v .. ' -t tmux -2', 3)
      end
]]--
      break
    end
